...
class SystemFunctionTest {
 ...
 fun `Should calculate full function`(x: Double) {
  ...
  val actual = SystemFunction().calculate(x, epsilon)
 ...
 fun `Should not calculate calculateTrigPart(x)`(x: Double) {
  ...
  assertThrows<IllegalArgumentException> {
   SystemFunction().calculateTrigPart(x, epsilon)
  }
 }
 ...
 fun `Should calculate calculateTrigPart(x)`(x: Double) {
  ...
  val actual = SystemFunction().calculateTrigPart(x, epsilon)
  assertTrue(abs(expected - actual) < epsilon, "Expected: $expected, actual: $actual")
 }
 ...
 fun `Should calculate calculateLogPart(x)`(x: Double) {
  ...
  val actual = SystemFunction().calculateLogPart(x, epsilon)
  assertTrue(abs(expected - actual) < epsilon, "Expected: $expected, actual: $actual")
 }
}